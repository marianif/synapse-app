...
import { signalFor } from "@/components/molecules/field-greeting";
...
export function OrbitConsole({
  ...
  /** Which channel is being read aloud — owned by the parent so the greeting,
      rendered as a sibling, reflects the same focus the dots set. */
  focusedType: EntryType | null;
  /** Toggle focus on a populated channel (re-tapping the focused one clears it). */
  onToggleFocus: (type: EntryType) => void;
}): React.ReactElement {
  ...
  // the uniform orbit; populated lets the dots breathe per channel signal.
        ...
        {CHANNELS.map((c, i) => {
          ...
          return (
            ...
            >
              <Pressable
                onPress={() =>
                  reactive ? onToggleFocus(c.type) : openChannel(c.type)
                }
                onPressIn={
                  reactive
                    ? () => {
