...
// orphan sweep: in-scope files the dependency-order dropped from all phases.
const scope = JSON.parse(fs.readFileSync(".impeccable/migration-scope.json","utf-8"));
const allScope = new Set([...scope.screens.map(s=>s.file), ...scope.components]);
const phased = new Set(brief.dependencyOrder.phases.flatMap(p=>p.files));
const phaseFiles = [...allScope].filter(f=>!phased.has(f));
...
for (const file of phaseFiles) {
  ...
  if (src !== orig) {
    ...
    if (!/migrated to v2 tokens — phase orphans/.test(src)) {
      src = "// migrated to v2 tokens — phase orphans\n" + src;
    }
  }
...
const mdPath = path.join(rootDir, ".impeccable/generated/phase-orphans-rewrite-report.md");
